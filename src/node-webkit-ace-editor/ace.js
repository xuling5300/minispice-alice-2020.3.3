(function () {
    ...
    (function () {
        r.implement(this, p), this.setKeyboardHandler = function (e) {
            ...
            if (!this.selection.isEmpty())
                r = this.session.remove(this.getSelectionRange()), this.clearSelection();
            ...
            if (t.getDocument().isNewLine(e)) {
                ...
                for (var v = r.row + 1; v <= l.row; ++v) {
                    var y = d;
                    a = t.getLine(v);
                    for (var g = 0; g < a.length && y > 0; ++g)
                        a.charAt(g) == "	" ? y -= p : a.charAt(g) == " " && (y -= 1);
            ...
            if (this.$highlightSelectedWord == e)
                return;
            ...
            if (!n.isMultiLine())
                var r = n.getRange(), i = n.isBackwards();
    ...
    (function () {
        this.onMouseDown = function (e) {
            ...
            clipboard.set('This is node-webkit content write to ClipBoard', 'text'); //write something to clipboard
            clipboard.get('text'); // Read from clipboard
            ...
            var keywords = '';
            var currentEditorID = document.getElementById("editorID").value;
            //console.log("the current editor is:" + currentEditorID);
            numStart = $($("div #editorContainer_" + productname + " #" + currentEditorID + " .ace_gutter-cell ")[0]).text();//the begin line number of editor, alice add
            currentLineNumber = n.row - numStart + 1;
            keywords = $($("div #editorContainer_" + productname + " #" + currentEditorID + " .ace_line")[currentLineNumber]).text();//alice add
				
            var keytext = keywords.replace(/\s+/g, ' ').split(" ")[0];

            
            
            
            
            /* alice modify left and right click 以下为取屏幕关键字单词的逻辑*/
            var isNumber = true;
            if (keywords[0] === "#" || keywords[0] === "$" || keywords === " ") {
                isNumber = false;
            }
            keywords = keywords.replace(/\s/g, '@');//把空格都替换为@便于截取,这里有不明原因直接获取的行字符串不能直接截取使用
            ...
            //当前操作字符是否是数字（如果光标左右的字符都不是字符）
            if(isNaN(keywords.substr(n.column-1,1)) && isNaN(keywords.substr(n.column,1))){//
                isNumber = false;
            }
            //科学计数法都是数字
            if(keywords.substr(n.column-1,1)==="." && (keywords.substr(n.column,1)==="e" || keywords.substr(n.column,1)==="E")){
                isNumber = true;
            }
            if((keywords.substr(n.column-1,1)==="e" || keywords.substr(n.column-1,1)==="E") && (keywords.substr(n.column,1)==="+" || keywords.substr(n.column,1)==="-")){
                isNumber = true;
            }
            
            //取当前光标处到前后空格之间的单词
            var startNum = newn, endNum = newn;
            for (i = newn; i >= 0; i--) {
                if (keywords[i] === "@") {
                    break;
                } else {
            ...
            for (i = newn; i < keywords.length; i++) {
                if (keywords[i] === "@") {
                    break;
                } else {
                    endNum++;
            ...
            //鼠标点击的当前单词(前后空格之间的单词)
            var screenKeyWord = keywords.substring(startNum + 1, endNum);                

            //如果鼠标点击在单词第一个字符前，则取光标右侧的单词
            if (screenKeyWord === "@") {
                for (i = startNum + 1; i < keywords.length; i++) {
                    if (keywords[i] === "@") {
                        break;
                    } else {
                        endNum++;
                ...
                screenKeyWord = keywords.substring(startNum + 1, endNum + 1);
            }
            
            var obj = document.getElementById("rightclickpanel2");
            var str = "";
            var kname=keytext;                
            var variableName="";
            var currentValue="";
            ...
            //如果上一行结尾是&&，表示当前行没有命令关键字，则往上一行循环寻找关键字
            var lasttext = $($("div #" + currentEditorID + " .ace_line")[currentLineNumber-1]).text().replace(/(^\s*)|(\s*$)/g, '');
            if(lasttext.substring(lasttext.length-2) === "&&"){
                var keywordline = currentLineNumber-1;
                for(i=keywordline;i>1;i--){
                    lasttext = $($("div #" + currentEditorID + " .ace_line")[i]).text().replace(/(^\s*)|(\s*$)/g, '');
                    if(lasttext.substring(lasttext.length-2) !== "&&"){
                        break;
                    }else{
                        keywordline = i;
                ...
                kname = $($("div #" + currentEditorID + " .ace_line")[keywordline]).text().replace(/(^\s*)|(\s*$)/g, '').replace(/\s/g, '@').split("@")[0];
            }

            if (screenKeyWord.indexOf("=") !== -1) {//如果单词有等号
                variableName = screenKeyWord.split("=")[0];
                currentValue = screenKeyWord.split("=")[1];
                if(currentValue.indexOf("(")!==-1 || currentValue.indexOf("[")!==-1){
                    currentValue = currentValue.substring(1);
                    vPosition = 0;
                }
            }else{//如果单词没有等号(参数值离等号之间有多个空格，或这是多参数其中的一个)
                var tv = screenKeyWord;
                if(screenKeyWord.length>1)//除去最后一个字符
                    tv = screenKeyWord.substring(0,screenKeyWord.length-1);

                var lv = screenKeyWord.substring(screenKeyWord.length-1);
                //如果单词是数字, 或者除最后一个字符外是数字且最后一个字符是括号，则往前找等号并找到属性名
                if (!isNaN(screenKeyWord) || (!isNaN(tv) && (lv===")" || lv==="]" || lv===","))) {
                    var equalPositonEnd = startNum;
                    ...
                    var t1= equalPositonEnd-1;//属性名结束位置
                    for(i=equalPositonEnd-1;i>0;i--){
                        if(keywords[i] === "@"){
                            t1--;
                        }else{
                            break;
                    ...
                    var passv = [];//往前找等号时经历了几个参数，以此判断当前参数的位置
                    var isstart = true;
                    var passnum = false;
                    var denghao = startNum;
                    ...
                    vPosition = passv.length;

                    if(passv.length===0){//单一参数时参数名取值
                        if(t2!==0)t2++;
                        variableName = keywords.substring(t2, t1+1);
                    }else{//多参数时参数名的取值
                        var pstart = denghao;
                        for(i=denghao;i>0;i--){
                            if(keywords[i] !== "@")
                        ...
                        variableName = keywords.substring(pstart+1, denghao);
                    }

                    currentValue = screenKeyWord;
                    if(currentValue.indexOf("(")!==-1 || currentValue.indexOf("[")!==-1){
                        currentValue = currentValue.substring(1);
                    }  
                    if(lv===")" || lv==="]" || lv===","){
            ...
            var tempObj = {isNumber:isNumber,lineNumber:(n.row + 1),keywordName:kname,variableName: variableName,currentValue:currentValue,vPosition:vPosition};
            $("#fixNoRefresh").click();
            ...
            if (t && !r.isFocused()) {
                ...
                if (this.$focusWaitTimout && !this.$clickSelection)
                    return this.setState("focusWait"), this.captureMouse(e), e.preventDefault()
            }
        ...
        }, this.onClick = function (e) {//alice add: add click event            
    ...
    (function () {
        ...
        i.implement(this, u), this.setDocument = function (e) {
            ...
            for (var a = 1; a < e.length; a++)
                i = e[a], r(i) ? (o = i.range.start, s.compare(o.row, o.column) == -1 && s.setStart(i.range.start), o = i.range.end, s.compare(o.row, o.column) == 1 && s.setEnd(i.range.end), u = !0) : (o = i.range.start, s.compare(o.row, o.column) == -1 && (s = l.fromPoints(i.range.start, i.range.start)), u = !1);
            if (n != null) {
            ...
            if (this.$wrapLimitRange.min !== e || this.$wrapLimitRange.max !== t)
                this.$wrapLimitRange.min = e, this.$wrapLimitRange.max = t, this.$modified = !0, this._emit("changeWrapMode")
            ...
            while (c <= t) {
                ...
                if (!l)
                    f = this.$getDisplayTokens(s.stringTrimRight(n[c])), i[c] = this.$computeWrapSplits(f, u, r), c++;
                else {
                    f = [], l.walk(function (e, t, r, i) {
                        ...
                        if (e != null) {
                            ...
                            s = this.$getDisplayTokens(n[t].substring(i, r), f.length);
                        f = f.concat(s)
                    }.bind(this), l.end.row, n[l.end.row].length + 1);
                    while (f.length != 0 && f[f.length - 1] >= d)
                        f.pop();
        ...
        this.$computeWrapSplits = function (e, t) {
            ...
            for (var o = 0; o < e.length; o++) {
                ...
                if (u == 9) {
                    ...
                    u == 32 ? i.push(d) : u > 39 && u < 48 || u > 57 && u < 64 ? i.push(c) : u >= 4352 && g(u) ? i.push(t, n) : i.push(t)
            }
            return i
    ...
    (function () {
        this.$arrayTokens = function (e) {
            ...
            if (r.length != t.length - 2)
                return window.console && console.error(r.length, t.length - 2, e, this.splitRegex), [{type: "error.invalid", value: e}];
            ...
            while (u = o.exec(e)) {
                ...
                for (var m = 0; m < u.length - 2; m++) {
                    if (u[m + 1] === undefined)
                        continue;
                    //alice modify the below:  (can't find the reason of this bug!!)
                    h = i[s[m]], c = typeof h.token == "function" ? h.token(p, r, n) : h.token, h.next && (r = h.next, i = this.states[r], i || (window.console && console.error, r = "start", i = this.states[r]), s = this.matchMappings[r], f = d, o = this.regExps[r], o.lastIndex = d);
                ...
                if (p)
                    if (typeof c == "string")
                        !!h && h.merge === !1 || l.type !== c ? (l.type && a.push(l), l = {type: c, value: p}) : l.value += p;
                    else {
        ...
        this.addRules = function (e, t) {
            ...
            for (var t in this.$rules) {
                ...
                for (var r = 0; r < n.length; r++) {
                    ...
                    if (i.rules)
                        for (var o in i.rules)
                            this.$rules[o] ? this.$rules[o].push && this.$rules[o].push.apply(this.$rules[o], i.rules[o]) : this.$rules[o] = i.rules[o];
        ...
        this.MAX_RANGES = 500, this.setRegexp = function (e) {
            ...
            for (var a = o; a <= u; a++) {
                ...
                for (var l = f.length; l--; )
                    t.drawSingleLineMarker(e, f[l].toScreenRange(n), this.clazz, i, null, this.type)
            }
        }
    ...
    function u() {
        this.getFoldAt = function (e, t, n) {
            ...
            function n(t) {
                ...
                if (!t.subFolds)
                    return;
                for (var r = 0; r < t.subFolds.length; r++)
                    n(t.subFolds[r])
            }
            ...
            if (u) {
                ...
                s && this.foldAll(e + 1, this.getLength()), (t.target || t.srcElement).className += " ace_invalid"
    ...
    (function () {
        this.addCommand = function (e) {
            ...
            for (var o = t.length; o--; ) {
                ...
                if (u == null)
                    throw"invalid modifier " + t[o] + " in " + e;
                s |= u
            }
    ...
    t.commands = [{name: "selectall", bindKey: s("Ctrl-A", "Command-A"), exec: function (e) {
                e.selectAll()
            }, readOnly: !0}, {name: "centerselection", bindKey: s(null, "Ctrl-L"), exec: function (e) {
                e.centerSelection()
            }, readOnly: !0}, {name: "gotoline", bindKey: s("Ctrl-L", "Command-L"), exec: function (e) {
                var t = parseInt(prompt("Enter line number:"), 10);
                isNaN(t) || e.gotoLine(t)
            }, readOnly: !0}, {name: "fold", bindKey: s("Alt-L|Ctrl-F1", "Command-Alt-L|Command-F1"), exec: function (e) {
                e.session.toggleFold(!1)
            }, readOnly: !0}, {name: "unfold", bindKey: s("Alt-Shift-L|Ctrl-Shift-F1", "Command-Alt-Shift-L|Command-Shift-F1"), exec: function (e) {
                e.session.toggleFold(!0)
            }, readOnly: !0}, {name: "foldall", bindKey: s("Alt-0", "Command-Option-0"), exec: function (e) {
                e.session.foldAll()
            }, readOnly: !0}, {name: "unfoldall", bindKey: s("Alt-Shift-0", "Command-Option-Shift-0"), exec: function (e) {
                e.session.unfold()
            }, readOnly: !0}, {name: "findnext", bindKey: s("Ctrl-K", "Command-G"), exec: function (e) {
                e.findNext()
            }, readOnly: !0}, {name: "findprevious", bindKey: s("Ctrl-Shift-K", "Command-Shift-G"), exec: function (e) {
                e.findPrevious()
            }, readOnly: !0}, {name: "find", bindKey: s("Ctrl-F", "Command-F"), exec: function (e) {
                i.loadModule("ace/ext/searchbox", function (t) {
                    t.Search(e)
                })
            }, readOnly: !0}, {name: "overwrite", bindKey: "Insert", exec: function (e) {
                e.toggleOverwrite()
            }, readOnly: !0}, {name: "selecttostart", bindKey: s("Ctrl-Shift-Home", "Command-Shift-Up"), exec: function (e) {
                e.getSelection().selectFileStart()
            }, multiSelectAction: "forEach", readOnly: !0}, {name: "gotostart", bindKey: s("Ctrl-Home", "Command-Home|Command-Up"), exec: function (e) {
                e.navigateFileStart()
            }, multiSelectAction: "forEach", readOnly: !0}, {name: "selectup", bindKey: s("Shift-Up", "Shift-Up"), exec: function (e) {
                e.getSelection().selectUp()
            }, multiSelectAction: "forEach", readOnly: !0}, {name: "golineup", bindKey: s("Up", "Up|Ctrl-P"), exec: function (e, t) {
                e.navigateUp(t.times)
            }, multiSelectAction: "forEach", readOnly: !0}, {name: "selecttoend", bindKey: s("Ctrl-Shift-End", "Command-Shift-Down"), exec: function (e) {
                e.getSelection().selectFileEnd()
            }, multiSelectAction: "forEach", readOnly: !0}, {name: "gotoend", bindKey: s("Ctrl-End", "Command-End|Command-Down"), exec: function (e) {
                e.navigateFileEnd()
            }, multiSelectAction: "forEach", readOnly: !0}, {name: "selectdown", bindKey: s("Shift-Down", "Shift-Down"), exec: function (e) {
                e.getSelection().selectDown()
            }, multiSelectAction: "forEach", readOnly: !0}, {name: "golinedown", bindKey: s("Down", "Down|Ctrl-N"), exec: function (e, t) {
                e.navigateDown(t.times)
            }, multiSelectAction: "forEach", readOnly: !0}, {name: "selectwordleft", bindKey: s("Ctrl-Shift-Left", "Option-Shift-Left"), exec: function (e) {
                e.getSelection().selectWordLeft()
            }, multiSelectAction: "forEach", readOnly: !0}, {name: "gotowordleft", bindKey: s("Ctrl-Left", "Option-Left"), exec: function (e) {
                e.navigateWordLeft()
            }, multiSelectAction: "forEach", readOnly: !0}, {name: "selecttolinestart", bindKey: s("Alt-Shift-Left", "Command-Shift-Left"), exec: function (e) {
                e.getSelection().selectLineStart()
            }, multiSelectAction: "forEach", readOnly: !0}, {name: "gotolinestart", bindKey: s("Alt-Left|Home", "Command-Left|Home|Ctrl-A"), exec: function (e) {
                e.navigateLineStart()
            }, multiSelectAction: "forEach", readOnly: !0}, {name: "selectleft", bindKey: s("Shift-Left", "Shift-Left"), exec: function (e) {
                e.getSelection().selectLeft()
            }, multiSelectAction: "forEach", readOnly: !0}, {name: "gotoleft", bindKey: s("Left", "Left|Ctrl-B"), exec: function (e, t) {
                e.navigateLeft(t.times)
            }, multiSelectAction: "forEach", readOnly: !0}, {name: "selectwordright", bindKey: s("Ctrl-Shift-Right", "Option-Shift-Right"), exec: function (e) {
                e.getSelection().selectWordRight()
            }, multiSelectAction: "forEach", readOnly: !0}, {name: "gotowordright", bindKey: s("Ctrl-Right", "Option-Right"), exec: function (e) {
                e.navigateWordRight()
            }, multiSelectAction: "forEach", readOnly: !0}, {name: "selecttolineend", bindKey: s("Alt-Shift-Right", "Command-Shift-Right"), exec: function (e) {
                e.getSelection().selectLineEnd()
            }, multiSelectAction: "forEach", readOnly: !0}, {name: "gotolineend", bindKey: s("Alt-Right|End", "Command-Right|End|Ctrl-E"), exec: function (e) {
                e.navigateLineEnd()
            }, multiSelectAction: "forEach", readOnly: !0}, {name: "selectright", bindKey: s("Shift-Right", "Shift-Right"), exec: function (e) {
                e.getSelection().selectRight()
            }, multiSelectAction: "forEach", readOnly: !0}, {name: "gotoright", bindKey: s("Right", "Right|Ctrl-F"), exec: function (e, t) {
                e.navigateRight(t.times)
            }, multiSelectAction: "forEach", readOnly: !0}, {name: "selectpagedown", bindKey: "Shift-PageDown", exec: function (e) {
                e.selectPageDown()
            }, readOnly: !0}, {name: "pagedown", bindKey: s(null, "Option-PageDown"), exec: function (e) {
                e.scrollPageDown()
            }, readOnly: !0}, {name: "gotopagedown", bindKey: s("PageDown", "PageDown|Ctrl-V"), exec: function (e) {
                e.gotoPageDown()
            }, readOnly: !0}, {name: "selectpageup", bindKey: "Shift-PageUp", exec: function (e) {
                e.selectPageUp()
            }, readOnly: !0}, {name: "pageup", bindKey: s(null, "Option-PageUp"), exec: function (e) {
                e.scrollPageUp()
            }, readOnly: !0}, {name: "gotopageup", bindKey: "PageUp", exec: function (e) {
                e.gotoPageUp()
            }, readOnly: !0}, {name: "scrollup", bindKey: s("Ctrl-Up", null), exec: function (e) {
                e.renderer.scrollBy(0, -2 * e.renderer.layerConfig.lineHeight)
            }, readOnly: !0}, {name: "scrolldown", bindKey: s("Ctrl-Down", null), exec: function (e) {
                e.renderer.scrollBy(0, 2 * e.renderer.layerConfig.lineHeight)
                ...
                e.getSelection().selectLineEnd()
            }, multiSelectAction: "forEach", readOnly: !0}, {name: "togglerecording", bindKey: s("Ctrl-Alt-E", "Command-Option-E"), exec: function (e) {
                e.commands.toggleRecording(e)
            }, readOnly: !0}, {name: "replaymacro", bindKey: s("Ctrl-Shift-E", "Command-Shift-E"), exec: function (e) {
                e.commands.replay(e)
            }, readOnly: !0}, {name: "jumptomatching", bindKey: s("Ctrl-P", "Ctrl-Shift-P"), exec: function (e) {
                e.jumpToMatching()
            }, multiSelectAction: "forEach", readOnly: !0}, {name: "selecttomatching", bindKey: s("Ctrl-Shift-P", null), exec: function (e) {
                e.jumpToMatching(!0)
            }, readOnly: !0}, {name: "cut", exec: function (e) {
                var t = e.getSelectionRange();
                e._emit("cut", t), e.selection.isEmpty() || (e.session.remove(t), e.clearSelection())
            }, multiSelectAction: "forEach"}, {name: "removeline", bindKey: s("Ctrl-D", "Command-D"), exec: function (e) {
                e.removeLines()
            }, multiSelectAction: "forEach"}, {name: "duplicateSelection", bindKey: s("Ctrl-Shift-D", "Command-Shift-D"), exec: function (e) {
                e.duplicateSelection()
            }, multiSelectAction: "forEach"}, {name: "sortlines", bindKey: s("Ctrl-Alt-S", "Command-Alt-S"), exec: function (e) {
                e.sortLines()
            }, multiSelectAction: "forEach"}, {name: "togglecomment", bindKey: s("Ctrl-/", "Command-/"), exec: function (e) {
                e.toggleCommentLines()
            }, multiSelectAction: "forEach"}, {name: "modifyNumberUp", bindKey: s("Ctrl-Shift-Up", "Alt-Shift-Up"), exec: function (e) {
                e.modifyNumber(1)
            }, multiSelectAction: "forEach"}, {name: "modifyNumberDown", bindKey: s("Ctrl-Shift-Down", "Alt-Shift-Down"), exec: function (e) {
                e.modifyNumber(-1)
            }, multiSelectAction: "forEach"}, {name: "replace", bindKey: s("Ctrl-H", "Command-Option-F"), exec: function (e) {
                i.loadModule("ace/ext/searchbox", function (t) {
                    t.Search(e, !0)
                })
                ...
                e.remove("right")
            }, multiSelectAction: "forEach"}, {name: "backspace", bindKey: s("Command-Backspace|Option-Backspace|Shift-Backspace|Backspace", "Ctrl-Backspace|Command-Backspace|Shift-Backspace|Backspace|Ctrl-H"), exec: function (e) {
                e.remove("left")
            }, multiSelectAction: "forEach"}, {name: "removetolinestart", bindKey: s("Alt-Backspace", "Command-Backspace"), exec: function (e) {
                e.removeToLineStart()
            }, multiSelectAction: "forEach"}, {name: "removetolineend", bindKey: s("Alt-Delete", "Ctrl-K"), exec: function (e) {
                e.removeToLineEnd()
            }, multiSelectAction: "forEach"}, {name: "removewordleft", bindKey: s("Ctrl-Backspace", "Alt-Backspace|Ctrl-Alt-Backspace"), exec: function (e) {
                e.removeWordLeft()
            }, multiSelectAction: "forEach"}, {name: "removewordright", bindKey: s("Ctrl-Delete", "Alt-Delete"), exec: function (e) {
                e.removeWordRight()
            }, multiSelectAction: "forEach"}, {name: "outdent", bindKey: s("Shift-Tab", "Shift-Tab"), exec: function (e) {
                e.blockOutdent()
            }, multiSelectAction: "forEach"}, {name: "indent", bindKey: s("Tab", "Tab"), exec: function (e) {
                e.indent()
            }, multiSelectAction: "forEach"}, {name: "blockoutdent", bindKey: s("Ctrl-[", "Ctrl-["), exec: function (e) {
                e.blockOutdent()
            }, multiSelectAction: "forEach"}, {name: "blockindent", bindKey: s("Ctrl-]", "Ctrl-]"), exec: function (e) {
                e.blockIndent()
            }, multiSelectAction: "forEach"}, {name: "insertstring", exec: function (e, t) {
                e.insert(t)
            }, multiSelectAction: "forEach"}, {name: "inserttext", exec: function (e, t) {
                e.insert(r.stringRepeat(t.text || "", t.times || 1))
            }, multiSelectAction: "forEach"}, {name: "splitline", bindKey: s(null, "Ctrl-O"), exec: function (e) {
                e.splitLine()
            }, multiSelectAction: "forEach"}, {name: "transposeletters", bindKey: s("Ctrl-T", "Ctrl-T"), exec: function (e) {
                e.transposeLetters()
            }, multiSelectAction: function (e) {
                e.transposeSelections(1)
            }}, {name: "touppercase", bindKey: s("Ctrl-U", "Ctrl-U"), exec: function (e) {
                e.toUpperCase()
            }, multiSelectAction: "forEach"}, {name: "tolowercase", bindKey: s("Ctrl-Shift-U", "Ctrl-Shift-U"), exec: function (e) {
                e.toLowerCase()
    ...
    (function () {
        this.showGutter = !0, this.CHANGE_CURSOR = 1, this.CHANGE_MARKER = 2, this.CHANGE_GUTTER = 4, this.CHANGE_SCROLL = 8, this.CHANGE_LINES = 16, this.CHANGE_TEXT = 32, this.CHANGE_SIZE = 64, this.CHANGE_MARKER_BACK = 128, this.CHANGE_MARKER_FRONT = 256, this.CHANGE_FULL = 512, this.CHANGE_H_SCROLL = 1024, r.implement(this, v), this.updateCharacterSize = function () {
            ...
            if (n && (e || this.resizing > 1 || o.width != n)) {
                ...
                if (this.session.getUseWrapMode() && this.adjustWrapLimit() || e)
                    s |= this.CHANGE_FULL
            }
            ...
            t -= e.offset;
            if (t < 0 || t > e.height - this.lineHeight)
                return;
            ...
            if (t < 0 && this.session.getScrollTop() > 0)
                return!0;
            if (t > 0 && this.session.getScrollTop() + this.$size.scrollerHeight < this.layerConfig.maxHeight)
                return!0
        ...
        this.$padding = 0, this.setPadding = function (e) {
            ...
            for (var n in this.markers) {
                ...
                if (r.renderer) {
                    ...
                    r.type == "fullLine" ? this.drawFullLineMarker(t, s, r.clazz, e) : s.isMultiLine() ? r.type == "text" ? this.drawTextMarker(t, s, r.clazz, e) : this.drawMultiLineMarker(t, s, r.clazz, e) : this.drawSingleLineMarker(t, s, r.clazz + " ace_start", e)
            }
            ...
            for (s = t.start.row + 1; s < t.end.row; s++)
                o.start.row = s, o.end.row = s, o.end.column = this.session.getScreenLastRowColumn(s), this.drawSingleLineMarker(e, o, n, i, 1, "text")
    ...
    (function () {
        this.updateSelectionMarkers = function () {
            ...
            for (var a = i.ranges.length; a--; )
                u.fromOrientedRange(i.ranges[a]), this.selection = n.selection = u, e.exec(this, t || {}), u.toOrientedRange(i.ranges[a]);
            ...
            if (this.inMultiSelectMode) {
                ...
                e = [];
                for (var n = 0; n < t.length; n++)
                    e.push(this.session.getTextRange(t[n]));
